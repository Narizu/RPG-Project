...
public class HunterAttack : MonoBehaviour
{
    ...
    public Rigidbody arrow;                                                                         // Flecha para la habilidad 1.
    public Rigidbody arrowSkill;                                                                    // Flecha para la habilidad 3.
    public Rigidbody arrowUlti;                                                                     // Flecha para la habilidad 5.
    ...
    Rigidbody arrowSelected;                                                                        // Flecha seleccionada.
    ...
    void Skill1()
    {
        ...
        anim.SetInteger("Attack", Random.Range(1, 5));                                              // Seleccionar un ataque.

        arrowSelected = arrow;                                                                      // Fijar flecha seleccionada.
    ...
    void Skill3()
    {
        anim.SetTrigger("AttackTrigger");                                                           // Activar la animación de ataque.
        anim.SetBool("IsAttacking", true);                                                          // Fijar la animación de ataque a verdadero.
        anim.SetInteger("Attack", 5);                                                               // Seleccionar un ataque.

        particle.SetActive(true);
        arrowSelected = arrowSkill;                                                                 // Fijar flecha seleccionada.
    ...
    void Skill5()
    {
        anim.SetTrigger("AttackTrigger");                                                           // Activar la animación de ataque.
        anim.SetBool("IsAttacking", true);                                                          // Fijar la animación de ataque a verdadero.
        anim.SetInteger("Attack", 6);                                                               // Seleccionar un ataque.

        particle.SetActive(true);
        arrowSelected = arrowUlti;                                                                  // Fijar flecha seleccionada.
    ...
    {
        Rigidbody arrowInstance = Instantiate(arrowSelected, arrowSpawn.position, transform.rotation);   // Crear una flecha con la posición y la rotación.
        arrowInstance.velocity = 20 * transform.forward;                                            // Aplicar una fuerza para mover la flecha.
    ...
    public void SkillBoost()
    {
        ...
        {
            playerStats.dexterity = 7;                                                              // Aumentar la destreza.
    ...
    IEnumerator WaitSeconds()
    {
        ...
        playerStats.dexterity = 5;                                                                  // Reducir la destreza.
        s4particle.SetActive(false);
    }

    bool InRange()
    {
        return Vector3.Distance(transform.position, selected.transform.position) < 11f;             // Cuando el jugador está en rango.
    }
